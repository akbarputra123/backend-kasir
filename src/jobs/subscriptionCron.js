...
const jalankanSubscriptionCron = () => {
    cron.schedule("* * * * *", async () => {
        try {
            console.log("⏳ Cron: cek subscription...");
                    ...
                    if (existing.length === 0) {
                        await db.execute(
                            ...
                            [
                                ...
                                `Langganan ${subscription.nama_paket || ""} telah berakhir. Silakan perpanjang langganan untuk terus menggunakan SIOPOS.`,
                                subscription.id_subscription,
                            ]
                        );
            ...
            for (const subscription of expiringSubscriptions) {
                ...
                await db.execute(
                    ...
                    [
                        ...
                        `Langganan ${subscription.nama_paket || ""} akan berakhir dalam ${daysRemaining} hari. Silakan lakukan perpanjangan agar layanan tetap aktif.`,
                        subscription.id_subscription,
                    ]
                );

                console.log(
                    `⚠️ Notifikasi hampir expired dibuat untuk owner ${subscription.id_owner}`
                );
            }
    ...
    console.log(
        "✅ Subscription cron aktif - berjalan setiap 1 menit"
    );
};
